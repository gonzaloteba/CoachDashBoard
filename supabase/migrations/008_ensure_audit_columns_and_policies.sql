-- Migration 008: Ensure all audit columns exist + service_role INSERT policy
-- Run this in Supabase SQL Editor if you haven't run migration 007

-- ============================================
-- Client columns for Auditoría Inicial data
-- ============================================
...
-- Foto inicial (missing from previous migrations)
ALTER TABLE clients ADD COLUMN IF NOT EXISTS initial_photo_url TEXT;

-- Profile fields (from migration 005)
...
-- Check-in extended columns (from migration 007)
...
-- ============================================
-- RLS policies for service_role (webhook)
-- ============================================

-- Allow service_role to INSERT clients (auto-create from audit)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE policyname = 'Service role can insert clients'
  ) THEN
...
-- Allow service_role to UPDATE clients
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Allow service_role to SELECT clients (for findClientByName)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE policyname = 'Service role can read clients'
  ) THEN
    CREATE POLICY "Service role can read clients" ON clients
